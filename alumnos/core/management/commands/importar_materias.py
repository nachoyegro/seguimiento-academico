...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        with open('docs/materias.csv', 'r', encoding="utf8") as csvfile:
            spamreader = csv.reader(csvfile, delimiter=';')
            fila = 0
            for row in spamreader:
                carrera = Carrera.objects.get(codigo=row[0])
                plan = PlanDeEstudio.objects.get(
                    nombre=row[1], carrera=carrera)
                # Lo relleno con ceros hasta llegar a 5
                codigo = row[2].zfill(5)
                materia, _ = Materia.objects.get_or_create(siglas=row[4])
                materia.nombre = row[3]
                materia.save()
                MateriaEnPlan.objects.create(
                    materia=materia, codigo=codigo, plan=plan)
                fila += 1
